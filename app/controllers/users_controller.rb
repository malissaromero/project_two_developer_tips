    ...
    if params[:user][:username].strip == ""
      ...
      if !User.find_by(username: params[:user][:username])
        if User.create(
          ...
          password_digest: BCrypt::Password.create(params[:user][:password].strip)
        )
        ...
        decoded_hash = BCrypt::Password.new(User.find_by(username: params[:user][:username]).password_digest)
